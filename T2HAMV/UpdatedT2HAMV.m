...
controlTutorialFolder = './T2 MRI/T2 MRI/Healthy-Control (Updated)';
gliomaTutorialTestingFolder = './InstructionFolder/Training/Glioma';
meningiomaTutorialTestingFolder = './InstructionFolder/Training/Meningioma';
imageFolder = './InstructionFolder/Tutorial Images';
...
%Pre-allocating array for optimization and to work with Pack&Go
imageArrayOArrayTutorial = cell(1,length(gliomaTutorialFiles));
gliomaNumbersT = zeros(1,length(gliomaTutorialFiles));
meningiomaNumbersT = zeros(1,length(meningiomaTutorialFiles));
controlNumbersT = zeros(1,length(controlTutorialFiles));
...
% Remove the zeroes entries from pre-allocated gliomaNumbersT array
gliomaNumbersT(gliomaNumbersT==0) = [];
...
% Remove the zeroes entries from pre-allocated meningiomaNumbersT array
meningiomaNumbersT(meningiomaNumbersT==0) = [];
...
for k = 1:length(controlTutorialFiles)
    ...
    if mod(k, CONTROLNUM) == 1
        ...
        controlNumbersT(k) = sNintT;
    else
        imageArrayOArrayTutorial{sNintT} = [imageArrayOArrayTutorial{sNintT}; controlFullFileNameT];
    end
end

%Remove the zeroes from pre-allocated controlNumbersT array
controlNumbersT(controlNumbersT==0) = [];

% Remove empty cells from imageArrayOArray
empty = cellfun('isempty',imageArrayOArrayTutorial);
imageArrayOArrayTutorial(:,all(empty,1)) = [];
...
% Initialize time variables; make sure this is NOT commented out when running experiment
...
totalFiles = length(thirdFiles) + length(meningiomaFiles) + length(controlFiles) ;
...
% Time delay in seconds; 0.001 for testing and 0.3 otherwise
% Input delay in seconds; 0.0001 for testing and 0.2 otherwise
% TIMEDELAY = .3;
