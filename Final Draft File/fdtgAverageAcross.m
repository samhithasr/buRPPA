...
%initialize length of similarity matrix 
totalLength = length(originalC) + length(originalM) + length(fillerO) + length(fillerA);
% Create a square matrix of the total size, initially filled with NaN
similarityMatrix = zeros(totalLength, totalLength);
countingMatrix = zeros(totalLength, totalLength);


for k = 1:fileArrayLength
    load(fileArray{k}, 'thirdPoints', 'meningiomaPoints', 'controlPoints', 'point_mapping', 'meningiomaNumbers', 'numberVarName', 'controlNumbers', 'thirdNumbers', 'originalMatrix');
    load(probabilityArray{k}, 'meningiomaAccuracyMatrix', 'matrixVarName', 'controlAccuracyMatrix', 'thirdAccuracyMatrix', 'gliomaAccuracyMatrix');
        [n, ~] = size(originalMatrix);
        if strcmp(numberVarName, 'oligodendrogliomaNumbers') % control = 10,11,12,13 meningioma = 5,6,7,8,9 third = 1,2,3,4
            controlAdd = 0;
            meningiomaAdd = 0;
            thirdAdd = 0;
            controlNumbers = [10,11,12,13];
            meningiomNumbers = [5,6,7,8,9];
            thirdNumbers = [1,2,3,4];
            [similarityMatrix, countingMatrix] = updateSimilarityMatrix(controlAdd, meningiomaAdd, thirdAdd, countingMatrix, similarityMatrix, originalMatrix, controlNumbers, meningiomaNumbers, thirdNumbers);
        end 
        if strcmp(numberVarName, 'oligoastrocytmaNumbers') % control = 10,11,12,13 meningioma = 5,6,7,8,9 third = 1,2,3,4
            controlAdd = 0;
            meningiomaAdd = 0;
            thirdAdd = 13;
            controlNumbers = [10,11,12,13];
        ...
        if strcmp(numberVarName, 'astrocytomaNumbers') % control = 9,10,11,12 meningioma = 4,5,6,7,8 third = 1,2,3
            controlAdd = 1;
            meningiomaAdd = 1;
            thirdAdd = 17;
            controlNumbers = [9,10,11,12];
            meningiomNumbers = [4,5,6,7,8];
            thirdNumbers = [1,2,3];
            [similarityMatrix, countingMatrix] = updateSimilarityMatrix(controlAdd, meningiomaAdd, thirdAdd, countingMatrix, similarityMatrix, originalMatrix, controlNumbers, meningiomaNumbers, thirdNumbers)
        end 
...
function [countingMatrix, similarityMatrix] = updateSimilarityMatrix(controlAdd, meningiomaAdd, thirdAdd, countingMatrix, similarityMatrix, originalMatrix, controlNumbers, meningiomaNumbers, thirdNumbers)
            [n, ~] = size(originalMatrix);            
            for i = 1:n
                for j = i+1:n
                    % Access the element at (i, j), which is the same as (j, i)
                    currentElement = originalMatrix(i, j);
                    if ismember(i, controlNumbers) && ismember(j,controlNumbers)
                        x = controlAdd;
                        y = controlAdd;
                    end 
                    if ismember(i, controlNumbers) && ismember(j,thirdNumbers)
                        x = controlAdd;
                        y = thirdAdd;
                    end 
                    if ismember(i,controlNumbers) && ismember(j,meningiomaNumbers)
                        x = controlAdd;
                        y = meningiomaAdd;
                    end
                    if ismember(i,meningiomaNumbers) && ismember(j, thirdNumbers)
                        x = meningiomaAdd;
                        y = thirdAdd;
                    end 
                    if ismember(i, meningiomaNumbers) && ismember(j, meningiomaNumbers)
                        x = meningiomaAdd;
                        y = meningiomaAdd;
                    end 
                    if ismember(i, thirdNumbers) && ismember(j, thirdNumbers)
                        x = thirdAdd;
                        y = thirdAdd;

                    end 

                    similarityElement = similarityMatrix(i+x,j+y);
                    currentCounter = countingMatrix(i+x,j+y);
                    newValue = ((similarityElement*currentCounter)+currentElement)/(currentCounter+1);
                    similarityMatrix(i+x, j+y) = newValue;
                    similarityMatrix(j+y, i+x) = newValue;
                    countingMatrix(i+x, j+y) = currentCounter+1;
                    countingMatrix(j+y, i+x) = currentCounter+1;
