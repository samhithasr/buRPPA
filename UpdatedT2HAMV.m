...
controlTutorialFolder = fullfile(originalTutorialFolderPath, 'T2 MRI', 'T2 MRI/Healthy-Control (Updated)');
...
% Construct the filename using the user's name and the current date for uniqueness
% Define the current date
currentDate = datetime('today', 'Format', 'dd-MMM-yyyy');

% Define the filename66
filename = sprintf('T2HAM_%d_%s_%s.mat', programNumber, userName, currentDate);

% Specify the directory where you want to save the file (e.g., user's desktop)
%Error: Error using save
%Cannot create 'T2HAM_1_Test5_23-Jul-2024.mat' because
%'C:\Users\chris\Desktop' does not exist.
%Error in UpdatedT2HAMV (line 832)
%save(fullFilePath, 'thirdPoints', 'meningiomaPoints', 'controlPoints', 'point_mapping', 'meningiomaNumbers', 'numberVarName', 'controlNumbers', 'thirdNumbers', 'dataMatrix');
